...
const hefc_handle_presence = { /* handle_name: true/false, */ };
const hefp_handle_presence = { /* handle_name: true/false, */ };
const hifp_handle_presence = { /* handle_name: true/false, */ };
...
class LInterface {
    constructor(hefc, role) {
        ...
        this._isHefpHandleAvailable = (handle) => this._ref
            && this._ref.current
            && this._ref.current.hefp
            && this._ref.current.hefp[handle];
        this._isHifpHandleAvailable = (handle) => this._ref
            && this._ref.current
    ...
    hefc = (handle, params) => {
        if (this._isHefcHandle(handle) && this._isHefcHandleAvailable(handle)) {
            ...
            throw Error(`Error of hefc ${handle}: defined: ${this._isHefcHandle(handle)}, available: ${this._isHefcHandleAvailable(handle)}`);
        };
    };
    hefp = (handle, params) => {
        if (this._isHefpHandle(handle) && this._isHefpHandleAvailable(handle)) {
            ...
            throw Error(`Error of hefp ${handle}: defined: ${this._isHefpHandle(handle)}, available: ${this._isHefpHandleAvailable(handle)}`);
        };
    };
    hifp = (handle, params) => {
        if (this._isHifpHandle(handle) && this._isHifpHandleAvailable(handle)) {
            ...
            throw Error(`Error of hifp ${handle}: defined: ${this._isHifpHandle(handle)}, available: ${this._isHifpHandleAvailable(handle)}`);
        };
    };
}
